...
start_freq = 4.8433  # start grequency [GHz]
stop_freq = 5.1128 # stop frequency [GHz] maximum 8.5GHZ for ZND
freq_points = 201 # frequency sweep points
...
for count,gate_voltage in enumerate(gate_pattern): # ramping up the gate voltage
	...
	if count == 0:
		...
		if count > 0:
			...
			plt.imshow(S_amp, origin = 'lower', aspect='auto', extent=extent, cmap='seismic', vmin = -43, vmax = -34)
