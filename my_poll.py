...
def R_measure(device_id, amplitude, out_channel, in_channel, time_constant, frequency, poll_length, device, daq, out_mixer_channel, bias_resistor):
    ...
    exp_setting = [['/%s/sigins/%d/ac'             % (device, in_channel), 0],
                   ['/%s/sigins/%d/range'          % (device, in_channel), 5*amplitude],
                   ['/%s/sigins/%d/diff'          % (device, in_channel), 1],
                   ...
                   ['/%s/sigouts/%d/range'         % (device, out_channel), 5*amplitude],
    ...
    measured_R = np.mean(np.abs(sample['x'] + 1j*sample['y']))*bias_resistor/(amplitude*1e3)
    ...
    measured = [measured_R, measured_phi, measured_duration]
