...
delta_t = 5 # measurement every 60s
...
while (not END):
	...
	plt.plot(TIME[:-1], np.diff(RESISTANCE)/np.diff(TEMPERATURE), '--b', marker='.',markersize = 1.5, linewidth=0.5, alpha=0.9)
	...
	while (time.time()-t0 < delta_t_) and (not END):
