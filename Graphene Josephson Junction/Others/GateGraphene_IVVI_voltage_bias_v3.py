...
for count,Vg in enumerate(Vglist):
    ...
    I_min = 1e9*V_bias/np.max(resistance_array)
    I_max = 1e9*V_bias/np.min(resistance_array)
    print('I_min =', I_min)
    print('I_max =', I_max)
    ...
    try: 
    	plt.title(prefix+'$I_{max}$:{:.1f}nA, $I_{min}$:{:.1f}nA'.format(I_min,I_max))
    except: 
    	pass
